...
const NeuralCanvas = () => {
  ...
  useEffect(() => {
    ...
    // Defer initialization to avoid blocking main thread (LCP/TBT optimization)
    const initThree = async () => {
      ...
      // Setup cleanup handlers within closure
      const cleanup = () => {
        ...
        if (rendererInstance) {
          ...
          if (el.contains(rendererInstance.domElement)) el.removeChild(rendererInstance.domElement);
        }
      };

      el.dataset.cleanup = "true"; // Mark as initialized
      el.addEventListener('DOMNodeRemoved', cleanup);

      animate();
    };

    // Start slightly delayed to let the rest of the page paint first
    setTimeout(initThree, 50);

    return () => {
      // Basic cleanup for React side, detail cleanup happens inside initThree
      if (animId) cancelAnimationFrame(animId);
      if (rendererInstance) {
        rendererInstance.dispose();
        if (el && el.contains(rendererInstance.domElement)) el.removeChild(rendererInstance.domElement);
      }
    };
  }, [isMobile]);
