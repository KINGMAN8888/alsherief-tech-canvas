...
const Certifications = () => {
  ...
  const fallbackItems: CertItem[] = (
    ...
    ...item,
    image: CERT_IMAGES[i] || CERT_IMAGES[0],
    color: CERT_COLORS[i] || CERT_COLORS[0],
  }));

  const activeCerts = dbCerts && dbCerts.length > 0
    ? dbCerts.map((cert, i) => {
      const idx = i % CERT_COLORS.length;
      return {
        ...cert,
        color: CERT_COLORS[idx]
      };
    })
    : fallbackItems;
  ...
  return (
    <section id="certifications" className="relative py-28 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-20 bg-pattern-hex-certs" />
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-8">
        ...
        >
          {isLoading ? (
            <div className="text-cyan-400 py-12 flex justify-center animate-pulse tracking-widest font-rajdhani uppercase text-sm w-full">Loading Certificates...</div>
          ) : activeCerts.map((cert) => (
            <motion.button
              key={cert.title}
              variants={cardVariants}
              onClick={() => setSelected(cert)}
              className="group relative flex flex-col card-interactive-panel w-full sm:w-[calc(50%-0.625rem)] lg:w-[calc(33.333%-0.833rem)] xl:w-[calc(25%-0.9375rem)] 2xl:w-[calc(20%-1rem)] text-left cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/60"
            >
              {/* Gradient top bar */}
              <div
              ...
              {/* Hover glow overlay */}
              <div
                className={`absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none bg-gradient-to-br ${cert.color} mix-blend-soft-light`}
              />
